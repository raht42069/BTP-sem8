...
from model import DerainingTransformer
...
def test_loop(net, data_loader, num_iter):
    net.eval()
    psnr_values, ssim_values = [], []  # Initialize lists to store PSNR and SSIM values
    ...
    with torch.no_grad():
        ...
        for rain, norain, name, h, w in test_bar:
            ...
            psnr_values.append(current_psnr.item())  # Store PSNR for this iteration
            ssim_values.append(current_ssim.item())  # Store SSIM for this iteration
            count += 1
    ...
    return total_psnr / count, total_ssim / count, ssim_values, psnr_values


def save_loop(net, data_loader, num_iter):
    print('save loop')
    global best_psnr, best_ssim
    val_psnr, val_ssim, psnr_values, ssim_values = test_loop(net, data_loader, num_iter)
    ...
    plt.savefig('{}/psnr_ssim_plot.png'.format(args.save_path))
    ...
    if val_psnr > best_psnr and val_ssim > best_ssim:
        ...
        print('model saving')
        print('{}/{}.pth'.format(args.save_path, args.data_name))
...
if __name__ == '__main__':
    ...
    model = DerainingTransformer(args.num_blocks, args.num_heads, args.channels, args.num_refinement, args.expansion_factor).cuda()
    if args.model_file:
